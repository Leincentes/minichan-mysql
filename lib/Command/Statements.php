...
class Statements extends Config implements IStatements{
     ...
     * @return static
     */
    public function distinct($distinct): static
    {
     ...
     * @return static
     */
    public function table($table, $alias = null, $prefix = false): static
    {
        if (is_string($table) && $table) {
            ...
            new InvalidTableArgumentException();
        }
     ...
     * @return static
     */
    public function join($table, $condition, $type = 'LEFT', $prefix = false): static
    {
        $alias = null;
        if (is_string($table) && $table) {
            $tableName = trim($table);
     ...
     * @return static
     */
    public function order($field, $order = null): static
    {
     ...
     * @return static
     */
    public function group($group, $order = null): static
    {
        if (!empty($order) && !in_array(strtoupper($order), array('DESC', 'ASC'))) {
     ...
     * @return static
     */
    public function limit($offset, $length = null): static
    {
     ...
     * @return mixed
     */
    public function getOptions($name = null): mixed
    {
     ...
     * @return static
     */
    public function where($condition, $value = null): static
    {
        if (is_string($condition)) {
            if (!is_null($value)) {
     ...
     * @return string
     */
    public function rawKey($key): string
    {
     ...
     * @return mixed
     */
    public function column($field): mixed
    {
        if (!is_string($field)) {
            return false;
        }
     ...
     * @return array
     */
    public function getList($data = null, $unbuffered = false): array
    {
        // Set options
        if (is_array($data)) {
     ...
     * @return mixed
     */
    public function query($sql, array $bindData = array(), $fetch = false, $unbuffered = false): mixed
    {
        $sql = $this->parseSqlTable($sql);
        if (empty($bindData)) {
            $bindData = $this->getBindData();
